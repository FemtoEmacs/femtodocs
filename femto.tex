...
\begin{itemize}
\item \verb|C-|$\kappa$ -- Press and release \keys{Ctrl}
  ...
  \end{quote}
\item \verb|C-x C-|$\kappa$ -- Keep \keys{Ctrl}
...
If $x\in{\bf Int}$, Lisp programmers\index{Type} say that $x$ has type
\label{type:definition} \index{Type!Int} {\bf integer}. They also
...
into a template. When there appears a comma in the 
template, Lisp evaluates the expression following
the comma and replaces it with the result. 
...
All human or computer languages have dialects.
One could say that English, German
and Swedish are dialects of the
same language. A speaker of Swedish
can understand easily a person speaking
Danish or Bokmål. Brazilians visit
Argentine  and maintain
heated conversations with local
people without problem. Portuguese
and Spanish are in fact dialects of
Galician.

In general, dialects have common
grammar,
the difference rests on part of
the vocabulary. Compare the sentences
``Sabes que yo te amo?'' in Spanish and
``Sabes que eu te amo?'' in Portuguese.
The only variation is on the word
``yo'' that means I,
as in ``I love you.'' The Brazilian
actor Wagner Moura learned twenty
or so Spanish words, and there he
was playing the role of the
Colombian Pablo Escobar.

If Portuguese and Spanish speakers
accepted that they speak the same
language, and decided to spend
one month or so learning the 50
words that are different in their basic
vocabularies, their common idiom
would have 800,000,000 speakers
in Europe, Asia, Africa and Americas.

Here is another example of dialectal
similarity:
The  Bokmål  sentence ``Vet du at jeg
elsker deg?'' sounds pretty much
alike the correspondent
 Danish ``Ved du, at jeg elsker dig?''
 Why Norwegian speakers don't accept
 that their language is, in fact, Danish?

 Did you know that Malay is very
 close to Indonesian? The
 two languages are even closer than
 Portuguese and Spanish. Hindi and
 Urdu are pretty similar too.

 The language that you have learned
 in this book is called Scheme.
 The library \verb|r5rs.scm| provides
 a bridge between Scheme and the
 femtolisp compiler. There are other
 Lisp dialects, such as Common Lisp
 and Racket.

 In this chapter, you will learn how
 to write a web application in pure
 femtolisp. Thus, you will learn both femtolisp
 and do something useful. But firstly,
 let us learn what cloud computing is.

 Cloud computing is an Internet based
 computing that provides shared
 processing resources on demand to people
 connected to a computational service.
 When a  company adheres to a cloud computing
 service, it avoids infrastructure costs,
 such as purchasing servers, and keep
 them in working conditions.

 The cloudorado site shows that the fees
 of a provider can be pretty steep.
 ...
 names that rise the interest of
 Web users. If one wants to offer
 medical consultancy, \verb|medicina.tips|
 is a good name.

 Now that Nia has a user name and a
 domain, she must install femtolisp
 in her cloud machine. Here is how
 she enters in the shell:
\begin{quote}
\begin{verbatim}
...
\end{verbatim}
\end{quote}
The 2222 port is used in the protocol
...
\end{verbatim}
The \verb|.htaccess| file must contain the specification
of the lisp file extension.

The terminal that Nia can use in the cloud is called
jailshell. The jailshell does not allow Nia
to access the \verb|/usr/local| folder.
Then she must send \verb|flisp| and
\verb|flisp.boot| to her cloud home.
\begin{verbatim}
...
compiler to her  \verb|nia@medicina.tips| home folder.
The -P2222 port is indicated with uppercase P.
As you certainly remember, the \verb|ssh| command
requires lowercase p.

Again, all that stuff is easier to do than to
explain. The best policy is to ask for help
from a computer science major.

\paragraph{Script.} A program that generates
web pages is called script.
...
\verb|qstring.script| line by line. If the line
...
reads a dialog content from the browser
User Interface, and passes it to
the Lisp the lisp \verb|(os.getenv "QUERY_STRING")|
function. For instance, if  user
types {\em Nia Vardalos} in the text field
and presses the \menu{send} button,
the  \verb|(os.getenv "QUERY_STRING")| command
assigns \verb|temp=Nia+Vardalos&btn=send| to
the \verb|str| variable.
...
folder, you will find the \verb|flisp|
interpreter that can be run idependently
from the editor. This is the very same
femtolisp compiler that was sent to
the cloud service provider. You could
...
\end{verbatim}
\end{quote}
On Linux, you can use the \verb|apt-get|
...
Let us assume that Nia has installed
the \verb|rlwrap| editor, and typed the
\verb|webscript.scm| generator of
listing~\ref{lst:webscript.scm}
and the \verb|lst:qstring.script| pattern
in the \verb|~/femtodocs/samples/| folder.
...
interaction with user is called Read Eval
Print Loop or REPL for short. The reason is that
Lisp reads an expression, evals it, print the
...
After taking care of these compatibility
...
\begin{tikzpicture}[>=stealth',shorten >=1pt,on grid,
  ...
  \node[state]         (q3) [right of=q1] {$q_3$};
    \node[state]         (r0) [below of= S] {\verb|'()|};
  \node[state]         (q2) [below of=q1] {$q_2$};
  
  \path[->] (S)  edge  (q1)
  (q1) edge [bend right] node {.,:?!} (q2)
  (q1) edge [loop above] node {blank} (q1)
    (q3) edge [bend left=60] node {blank} (q1)
  (q1) edge [->,bend left] node {a-z} (q3)
  (q1) edge [bend right] node {""} (r0);
  \path[->]
  (q3) edge [loop right] node {a-z} (q3)
  (q3) edge [bend left=10] node {,.?} (q1);
\end{tikzpicture}
