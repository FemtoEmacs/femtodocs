...
reaches the end of a file, any read
...
which is stored at the \verb|fileio.scm| file.
...
\end{quote}
procedure. When a file is read beyond
...
expressions, when the $c$ condition holds.
...
fills a node at \verb|(* i sz)| with
blank chars.
...
of the node, and loops through the node size.
...
snippets from a well known
poem into nodes 0, 1, 2, 3 and 4.
...
Consider the third clause that appears
in the cond-form that appears in the
definition of the \verb|(word c s xs)|
procedure. The clause peeks the next
char in the stream, but does not 
advance the file descriptor. If the
char is the end-of-file object,
the automaton has finished with the
file. Therefore, the clause reverses
the \verb|xs| accumulator and 
casts it into a string.
Why one cannot use \verb|io.getc|
...
I believe that the designers of femtolisp
has provided such a complete set of
char reading functions, because they
...
in femtolisp, instead of Scheme.
Since femtolisp was designed recently,
