...
femtoemacs commands by pressing the 
\keys{Esc}\keys{$l$} shortcut.

There are commands that are not associated to
 shortcut keys. 
For instance, the \verb|discard-undo-history|
scraps the undo tree. Press \keys{Esc}\keys{$x$}
and type \verb|discard-undo-history| at the
minibuffer prompt. Then, if you press \verb|C-u|,
nothing will happen, since the undo tree is empty.
Below you will find a fairly complete list
of femtoemacs commands.
...
C-x C-c          exit
C-x C-f          find-file
C-x C-l          user-defined-function
C-x C-n          next-buffer
C-x C-s          save-buffer
C-x =            cursor-position
C-x C-u          user-defined-function
C-x C-w          write-file
C-x ?            describe-key
C-x i            insert-file
C-x k            kill-buffer
